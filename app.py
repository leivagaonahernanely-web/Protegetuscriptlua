...
def owner_generate_keys():
    ...
    for lic in licenses:
        ...
        keys_data.append({'key': lic.key, 'script_name': script.name if script else 'Desconocido', 'hwid': lic.hwid, 'expires_at': lic.expires_at, 'active': lic.active, 'max_uses': 'Ilimitado', 'used_count': lic.used_count, 'created_at': lic.created_at})
    return render_template('keys.html', user=current_user, keys=keys_data, scripts=scripts, owner_mode=True)
