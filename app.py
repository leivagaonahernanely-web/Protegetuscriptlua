...
# ✅ SOLUCIÓN DEFINITIVA: Fuerza HTTPS en todo el sistema
...
app.config['SESSION_COOKIE_NAME'] = 'LuaProtect_Session'
...
# ✅ OAuth CORREGIDO: URL escrita manualmente para evitar errores
oauth = OAuth(app)
...
local dec_ok, code = pcall(function() return HttpService:Decompress(HttpService:Base64Decode(res)) end)
...
def callback():
    try:
        ...
        if not user:
            user = User(discord_id=discord_id, username=username, avatar=avatar, is_admin=(discord_id == ADMIN_DISCORD_ID))
            db.session.add(user)
...
def new_script():
    ...
    nuevo = Script(name=nombre, description=descripcion, hash_id=hash_id, content=contenido, original_size=len(codigo), owner_id=current_user.id)
...
def new_license():
    ...
    if not Script.query.filter_by(hash_id=script_hash).first():
        flash("❌ Script no existe", "danger")
        return redirect(url_for('dashboard'))
    clave = generar_clave_licencia()
    expira = datetime.datetime.utcnow() + datetime.timedelta(days=dias)
    nueva = License(key=clave, script_hash=script_hash, max_uses=usos, expires_at=expira, created_by=current_user.id)
...
def toggle_license(key):
    ...
    db.session.commit()
    flash(f"✅ Licencia {'activada' if lic.active else 'desactivada'}", "success")
    return redirect(url_for('dashboard'))

# ---------------- API PARA CARGA DE SCRIPTS ----------------
...
def cargar_script(hash_id):
    ...
    if lic.uses >= lic.max_uses:
        ...
        db.session.commit()
        return Response(base64.b64encode(zlib.compress("error = 'Límite de usos'".encode())), mimetype='text/plain')
