...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv("DATABASE_URL", "sqlite:///vantaprotect_ultimate.db")
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('vantaprotect.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("VantaProtect")
...
class User(UserMixin, db.Model):
    ...
    panel_title = db.Column(db.String(255), default="VantaProtect")
    panel_description = db.Column(db.Text, default="Script protegido con VantaProtect")
    panel_guild_id = db.Column(db.String(32))
    panel_channel_id = db.Column(db.String(32))
    panel_message_id = db.Column(db.String(32))
...
class License(db.Model):
    ...
    discord_id = db.Column(db.String(32), index=True)
    role_id = db.Column(db.String(32), index=True)
...
class RolePermission(db.Model):
    ...
    role_id = db.Column(db.String(32), nullable=False, unique=True, index=True)
...
class PriceConfig(db.Model):
    ...
    title = db.Column(db.String(255), nullable=False, default='VantaProtect Prices')
    description = db.Column(db.Text, nullable=False, default='Planes y precios de VantaProtect.')
    body = db.Column(db.Text, nullable=False, default='Contacta al owner para conocer los planes disponibles.')
    channel_id = db.Column(db.String(32))
    color = db.Column(db.String(16), default='#8b5cf6')
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

class WebPlanKey(db.Model):
    __tablename__ = 'web_plan_keys'
    id = db.Column(db.Integer, primary_key=True)
    ...
    redeemed_by = db.Column(db.Integer, db.ForeignKey('users.id'))
...
def owner_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or str(current_user.discord_id) != str(ADMIN_DISCORD_ID):
            abort(403)
...
def generar_clave() -> str:
    return ''.join(random.SystemRandom().choices(string.ascii_letters + string.digits, k=32))

def comprimir_codigo(codigo: str) -> str:
    return base64.b64encode(zlib.compress(codigo.encode('utf-8'), level=9)).decode('utf-8')

def proteger_codigo(codigo: str) -> Tuple[str, Dict[str, int]]:
    protegido, stats = protect_lua(codigo)
    return comprimir_codigo(protegido), stats
...
@app.route('/owner/sources')
@login_required
@owner_required
def owner_sources():
    sources = SourceText.query.order_by(SourceText.updated_at.desc()).all()
    return render_template('sources.html', user=current_user, sources=sources)

@app.route('/owner/generate-keys')
@login_required
@owner_required
def owner_generate_keys():
    plan_keys = WebPlanKey.query.order_by(WebPlanKey.created_at.desc()).all()
    return render_template('web_keys.html', user=current_user, plan_keys=plan_keys)

@app.route('/api/owner/web-plan-keys', methods=['POST'])
@login_required
@owner_required
def generate_web_plan_key():
    try:
        data = request.get_json(silent=True) or {}
        plan = str(data.get('plan', 'PRO')).strip().upper()
        if plan not in {'PRO', 'PREMIUM'}:
            return jsonify({'success': False, 'error': 'Plan inválido'}), 400
        duration = str(data.get('duration', '30d')).strip()
        amount = max(1, min(int(data.get('amount', 1)), 100))
        generated=[]
        for _ in range(amount):
            key=generar_clave()
            while WebPlanKey.query.filter_by(key=key).first():
                key=generar_clave()
            item=WebPlanKey(key=key, plan=plan, expires_at=calcular_expiracion(duration), created_by=current_user.id)
            db.session.add(item)
            generated.append(key)
        db.session.commit()
        return jsonify({'success': True, 'keys': generated, 'plan': plan, 'amount': len(generated)})
    except Exception as e:
        db.session.rollback()
        logger.exception('Error generando keys de plan web')
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/owner/web-plan-keys/<int:key_id>', methods=['DELETE'])
@login_required
@owner_required
def delete_web_plan_key(key_id):
    item=WebPlanKey.query.get_or_404(key_id)
...
@owner_required
def owner_prices():
    prices = db.session.get(PriceConfig, 1) or PriceConfig(id=1)
    return render_template('prices.html', user=current_user, prices=prices)

@app.route('/api/owner/sources', methods=['POST'])
@login_required
@owner_required
def upload_source():
    try:
        uploaded = request.files.get('file')
        if not uploaded or not uploaded.filename or not uploaded.filename.lower().endswith('.txt'):
            return jsonify({'success': False, 'error': 'Solo se permiten archivos .txt'}), 400
        filename = re.sub(r'[^A-Za-z0-9._-]', '_', uploaded.filename)[:255]
        content = uploaded.read().decode('utf-8', errors='replace')
        source = SourceText.query.filter_by(filename=filename).first()
        if source:
            source.content = content
            source.uploaded_by = current_user.id
        else:
            source = SourceText(filename=filename, content=content, uploaded_by=current_user.id)
            db.session.add(source)
        db.session.commit()
        return jsonify({'success': True, 'filename': filename})
    except Exception:
        db.session.rollback()
        logger.exception('Error guardando source TXT')
        return jsonify({'success': False, 'error': 'No se pudo guardar el archivo'}), 500

@app.route('/api/owner/sources/<int:source_id>', methods=['GET', 'DELETE'])
@login_required
@owner_required
def manage_source(source_id):
    source = SourceText.query.get_or_404(source_id)
    if request.method == 'DELETE':
        db.session.delete(source)
        db.session.commit()
        return jsonify({'success': True})
    return source.content, 200, {'Content-Type': 'text/plain; charset=utf-8', 'Content-Disposition': f'attachment; filename="{source.filename}"'}

@app.route('/api/owner/prices', methods=['GET', 'POST'])
@login_required
def prices_api():
    if request.method == 'POST' and (not current_user.is_authenticated or str(current_user.discord_id) != str(ADMIN_DISCORD_ID)):
        return jsonify({'success': False, 'error': 'Owner only'}), 403
        prices = db.session.get(PriceConfig, 1) or PriceConfig(id=1)
    if request.method == 'POST':
        data = request.get_json(silent=True) or {}
        prices.title = str(data.get('title', prices.title))[:255]
        prices.description = str(data.get('description', prices.description))[:2000]
        prices.body = str(data.get('body', prices.body))[:10000]
        prices.channel_id = str(data.get('channel_id', prices.channel_id or ''))[:32] or None
        prices.color = str(data.get('color', prices.color or '#8b5cf6'))[:16]
        db.session.add(prices)
        db.session.commit()
    return jsonify({'success': True, 'prices': {'title': prices.title, 'description': prices.description, 'body': prices.body, 'channel_id': prices.channel_id, 'color': prices.color}})

@app.route('/prices')
def public_prices():
    prices = db.session.get(PriceConfig, 1) or PriceConfig(id=1)
    return render_template('public_prices.html', prices=prices)
...
@app.route('/scripts/hosted/<hash_id>.lua')
def public_script_loader(hash_id):
...
def load_script(hash_id):
    try:
        ...
        decoded = descomprimir_codigo(script.content)
        if decoded is None:
            try:
                decoded = base64.b64decode(script.content).decode('utf-8')
            except Exception:
                return "error: contenido protegido inválido", 500
        return decoded
    except Exception as e:
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 VantaProtect Ultimate iniciado en puerto {port}")
