...
@bot.tree.command(name="hwidban", description="Ban a HWID from this project")
...
async def hwidban(i: discord.Interaction, hwid: str, reason: str = "Blocked by manager"):
    ...
    with m["app"].app_context():
        ...
        if not ctx:
            return
        _, _, owner = ctx
        hwid = hwid.strip().strip("{}")
        existing = m["HWIDBan"].query.filter_by(hwid=hwid).first()
        if existing and existing.created_by != owner.id:
            await i.response.send_message("That HWID is already banned by another owner.", ephemeral=True)
            return
        if not existing:
            # El ban vale solo para los scripts de este dueño (created_by = dueño del script).
            m["db"].session.add(m["HWIDBan"](hwid=hwid, reason=reason[:255], created_by=owner.id))
            m["db"].session.commit()
    await i.response.send_message(f"⛔ HWID blacklisted.\nHWID: `{hwid}`\nReason: {reason}")


@bot.tree.command(name="unbanhwid", description="Unban a HWID from this project")
...
async def unbanhwid(i: discord.Interaction, hwid: str):
    ...
    with m["app"].app_context():
        ...
        if not ctx:
            return
        _, _, owner = ctx
        ban = m["HWIDBan"].query.filter_by(hwid=hwid.strip().strip("{}"), created_by=owner.id).first()
        if ban:
